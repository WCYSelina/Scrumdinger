...
struct CardView: View {
    ...
    var body: some View {
        VStack(alignment: .leading) {
            Text(scrum.title).font(.headline).padding(.bottom,6).accessibilityAddTraits(.isHeader)
            HStack {
                Label("\(scrum.attendees.count)", systemImage: "person.3").accessibilityLabel("\(scrum.attendees.count)attendees")
                Spacer()
                Label("\(scrum.lengthInMinutes)", systemImage: "clock").accessibilityLabel("\(scrum.lengthInMinutes)minutes")
